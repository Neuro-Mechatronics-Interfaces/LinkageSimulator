...
and renders only the state selected by the active tab. Servo angle, animation
direction, linkage edits, contactors, joint-limit status, and static-moment
results therefore persist independently for each digit. A shared hand-size
factor $h$ applies to all four anatomical models. Neutral initialization uses
rounded between-digit scales
\begin{equation}
...
$d$ is initialized with
\begin{equation}
  L_{i,d}=h s_d L_{i,2}.
\end{equation}
The linkage template is scaled by $s_d$ at creation so the four tabs retain the
same dimensionless mechanism geometry. Subsequent hardware edits remain local
to their digit and are not automatically resized when $h$ changes.
...
residual. Thus one blocked joint does not freeze the whole assembly. The tuned
default linkage uses a servo range of $38^\circ$ through $180^\circ$; fine-step
tests exercise the entire interval for D2--D5 and reach full configured MCP
extension, approximately $85^\circ$ MCP flexion, and more than $94^\circ$ PIP
flexion. This is a positional IK; no artificial stiffness is required. Rendering places a
rectangular pad dorsal to each digit segment and a U-shaped band down to the
flexor side.

Every rectangular link is also tested against each phalanx capsule, except when
the link carries the contactor explicitly assigned to that same phalanx. That
pair is treated as unilateral actuator contact: attempted overlap loads and
poses the digit rather than invalidating the whole assembly. For every other link--segment pair, link
...
explicit units, the shared overall hand scale, the active tab, and complete D2
through D5 states. Each digit entry includes its mechanism and hand
configuration, independent servo pose, contactor geometry, active joint-limit
states, and gravity-moment results. The export is a detached snapshot, so
subsequent animation does not mutate already generated data. Angles remain in
radians and lengths remain in millimetres.
